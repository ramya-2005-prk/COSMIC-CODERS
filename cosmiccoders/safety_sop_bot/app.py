    ...
    if not api_key:
        ...
        try:
            with st.spinner("Analyzing safety protocols..."):
                ...
                # Create chat session from history (excluding system prompt as that's in init)
                # Gemini Python SDK handles history automatically if we use start_chat, 
                # but for Streamlit statelessness, we often reconstruct.
                # Here we reconstruct the history for the API call.
                
                chat = model.start_chat(history=[])
                
                # Replay history to build context (skip the first greeting if it was hardcoded and not from API)
                # Actually, sending full history is better for context.
                ...
                for m in st.session_state.messages:
                    if m["content"] == "Hello! I can explain the LOTO procedures, PPE requirements, and Emergency Shutdown rules. What would you like to know?":
                        continue # Skip the hardcoded greeting to avoid confusing the model or just let it see it as its own previous turn
                    
                    role = "model" if m["role"] == "assistant" else "user"
                    history_for_gemini.append({"role": role, "parts": [m["content"]]})
                
                # Send the message
                # If history is empty, chat.send_message is fine. If not, we could set history.
                # For simplicity in this stateless app loop, let's just use generate_content with the full list including the new prompt.
                
                # Add the current prompt
                history_for_gemini.append({"role": "user", "parts": [prompt]})
